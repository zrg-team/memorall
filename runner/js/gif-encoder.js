// Vendored, worker-free GIF encoder (main thread only).
// Based on gif.js 0.2.0 (https://github.com/jnordberg/gif.js) - MIT License.
// Worker wrapper removed; runs synchronously in the calling context.
(function (global) {
  var ncycles = 100;
  var netsize = 256;
  ...
  GIFEncoder.prototype.ditherPixels = function (kernel, serpentine) {
    ...
    if (!kernel || !kernels[kernel]) throw new Error("Unknown dithering kernel: " + kernel);
  ...
  GIFEncoder.prototype.findClosest = function (c, used) {
    return this.findClosestRGB((c & 0xff0000) >> 16, (c & 0xff00) >> 8, c & 0xff, used);
  };
  ...
  GIFEncoder.prototype.writeLSD = function () {
    this.writeShort(this.width);
    this.writeShort(this.height);
    this.out.writeByte(0x80 | 0x70 | this.palSize);
    this.out.writeByte(0);
    this.out.writeByte(0);
  };
  ...
  GIFEncoder.prototype.toBlob = function () {
    ...
    var cursor = this.out.cursor;
    var pageSize = ByteArray.pageSize;
    var totalLength = (pages.length - 1) * pageSize + cursor;
    var data = new Uint8Array(totalLength);
    var offset = 0;
    for (var i = 0; i < pages.length; i++) {
      var len = i === pages.length - 1 ? cursor : pageSize;
      data.set(pages[i].subarray(0, len), offset);
      offset += len;
    }
