  ...
  try {
    switch (type) {
      ...
      case 'models': {
        const modelInfo = {
          object: 'list',
          data: [{ id: embeddingModel, name: embeddingModel, loaded: !!hfPipeline, object: 'model', created: Date.now(), owned_by: 'local' }]
        };
      ...
      case 'embeddings': {
        ...
        const response = {
          object: 'list',
          data: list.map((vec, idx) => ({ object: 'embedding', embedding: vec, index: idx })),
