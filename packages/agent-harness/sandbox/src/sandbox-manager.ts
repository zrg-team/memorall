import type { HarnessPlatform, JsonValue } from "@memorall/agent-harness-core";
import type {
...
import type { SandboxProviderRegistry } from "./provider-registry.js";
import { SandboxWorkspaceCoordinator } from "./workspace-coordinator.js";
...
export class SandboxManager implements IAgentSandboxService {
	...
	constructor(
		...
		private readonly platform: HarnessPlatform,
	...
	private callContext(
		...
		if (context?.deadlineMs !== undefined && this.platform.now() > context.deadlineMs) {
		...
		return {
			operationId: context?.operationId ?? `${operation}:${this.platform.randomUUID()}`,
