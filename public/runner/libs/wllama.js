...
var checkEnvironmentCompatible = () =>
	__async(void 0, null, function* () {
...
var CacheManager = class {
			...
			return yield new Promise((resolve, reject) => {
				...
				worker.onmessage = (e) => {
					...
					if (e.data.ok) {
						...
						(_a = options.progressCallback) == null
							? void 0
							: _a.call(options, progress);
					} else {
	...
	list() {
		return __async(this, null, function* () {
			...
			try {
				for (
					var iter = __forAwait(cacheDir.entries()), more, temp, error;
			...
			try {
				for (
					var iter2 = __forAwait(cacheDir.entries()), more2, temp2, error2;
...
function getCacheDir() {
	return __async(this, null, function* () {
		...
		const cacheDir = yield opfsRoot.getDirectoryHandle("cache", {
			create: true,
		});
		return cacheDir;
	});
...
var Model = class {
	...
	refresh() {
		return __async(this, arguments, function* (options = {}) {
			...
			const nParallel =
				(_a = this.modelManager.params.parallelDownloads) != null
					? _a
						...
						yield this.modelManager.cacheManager.download(
							...
							__spreadProps(__spreadValues({}, options), {
								progressCallback: ({ loaded }) => {
									...
									(_a2 = options.progressCallback) == null
										? void 0
										: _a2.call(options, {
...
var ModelManager = class _ModelManager {
	...
	static parseModelUrl(modelUrl) {
		...
		const queryParams =
			(_a = queryMatch == null ? void 0 : queryMatch[1]) != null ? _a : "";
	...
	getModels() {
		return __async(this, arguments, function* (opts = {}) {
			...
			if (!opts.includeInvalid) {
				models = models.filter((m) => m.validate() === "valid" /* VALID */);
			}
			return models;
		});
	}
	...
	getModelOrDownload(_0) {
		return __async(this, arguments, function* (url, options = {}) {
			...
			if (model) {
				(_a = options.progressCallback) == null
					? void 0
					: _a.call(options, { loaded: model.size, total: model.size });
				return model;
			}
...
var Wllama = class {
	constructor(pathConfig, wllamaConfig = {}) {
		...
		this.cacheManager =
			(_a = wllamaConfig.cacheManager) != null
				? _a
				: new cache_manager_default();
		this.modelManager =
			(_c = wllamaConfig.modelManager) != null
				? _c
				: new ModelManager({
	...
	loadModelFromUrl(_0) {
		return __async(this, arguments, function* (modelUrl, config = {}) {
			...
			const model = useCache
				? yield this.modelManager.getModelOrDownload(url, config)
				: yield this.modelManager.downloadModel(url, config);
	...
	createChatCompletion(messages, options) {
		return __async(this, null, function* () {
			...
			return options.stream
				? yield this.createCompletionGenerator(prompt, options)
				: yield this.createCompletion(
						prompt,
						__spreadProps(__spreadValues({}, options), { stream: false }),
					);
		});
	}
	...
	createCompletionImpl(prompt, options) {
		return __async(this, null, function* () {
			...
			for (
				let i = 0;
				i < ((_c = options.nPredict) != null ? _c : Infinity);
				i++
			) {
				...
				if (
					abort ||
					((_d = options.abortSignal) == null ? void 0 : _d.aborted)
				) {
					break;
				}
	...
	createCompletionGenerator(prompt, options) {
		return new Promise((resolve, reject) => {
			const createGenerator = cbToAsyncIter((callback) => {
	...
	getVocab() {
		return __async(this, null, function* () {
			this.checkModelLoaded();
			const result = yield this.proxy.wllamaAction("get_vocab", {
				_name: "gvoc_req",
			});
			return result.vocab;
		});
	...
	lookupToken(piece) {
		return __async(this, null, function* () {
			this.checkModelLoaded();
			const result = yield this.proxy.wllamaAction("lookup_token", {
				_name: "lkup_req",
				piece,
			});
			if (!result.success) {
	...
	decode(tokens, options) {
		return __async(this, null, function* () {
			...
			for (let i = 0; i < batches.length; i++) {
				if (
					(_a = options == null ? void 0 : options.abortSignal) == null
						? void 0
						: _a.aborted
				) {
	...
	samplingSample() {
		return __async(this, null, function* () {
			this.checkModelLoaded();
			const result = yield this.proxy.wllamaAction("sampling_sample", {
				_name: "ssam_req",
			});
			return {
				piece: result.piece,
	...
	samplingAccept(tokens) {
		return __async(this, null, function* () {
			this.checkModelLoaded();
			const result = yield this.proxy.wllamaAction("sampling_accept", {
				_name: "sacc_req",
				tokens,
			});
			if (!result.success) {
	...
	getLogits(topK = 40) {
		return __async(this, null, function* () {
			this.checkModelLoaded();
			const result = yield this.proxy.wllamaAction("get_logits", {
				_name: "glog_req",
				top_k: topK,
			});
			const logits = [];
	...
	embeddings(tokens) {
		return __async(this, null, function* () {
			...
			const result = yield this.proxy.wllamaAction("embeddings", {
				_name: "gemb_req",
				tokens,
			});
			if (!result.success) {
	...
	kvRemove(nKeep, nDiscard) {
		return __async(this, null, function* () {
			...
			const result = yield this.proxy.wllamaAction("kv_remove", {
				_name: "kvcr_req",
				n_keep: nKeep,
				n_discard: nDiscard,
			});
	...
	kvClear() {
		return __async(this, null, function* () {
			this.checkModelLoaded();
			const result = yield this.proxy.wllamaAction("kv_clear", {
				_name: "kvcc_req",
			});
			if (!result.success) {
	...
	formatChat(messages, addAssistant, template) {
		return __async(this, null, function* () {
			...
			const result = yield this.proxy.wllamaAction("chat_format", {
				_name: "cfmt_req",
				roles,
				contents,
				tmpl: template,
	...
	_testBenchmark(type, nSamples) {
		return __async(this, null, function* () {
			this.checkModelLoaded();
			return yield this.proxy.wllamaAction("test_benchmark", {
				_name: "tben_req",
				type,
				n_samples: nSamples,
			});
	...
	_testPerplexity(tokens) {
		return __async(this, null, function* () {
			this.checkModelLoaded();
			return yield this.proxy.wllamaAction("test_perplexity", {
				_name: "tper_req",
				tokens,
			});
		});
	...
	getCachedTokens() {
		return __async(this, null, function* () {
			this.checkModelLoaded();
			const result = yield this.proxy.wllamaAction("current_status", {
				_name: "stat_req",
			});
			return result.tokens;
		});
