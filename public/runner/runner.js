...
function setupWllamaRunner() {
	...
	async function ensureWllama() {
		...
		const mod = await import(
			"https://cdn.jsdelivr.net/npm/@wllama/wllama@2.3.5/esm/index.min.js"
		);
		...
		try {
			switch (type) {
				...
				case "serve": {
					...
					if (!filename) {
						throw new Error(
							"Model name must include filename: username/repo/filename",
						);
					}
					...
					await wllama.loadModelFromHF(username + "/" + repo, filename, {
						progressCallback,
					});
					const modelInfo = {
						id: modelId,
				...
				case "chat/completions": {
					...
					if (loadedModels.size === 0)
						throw new Error(
							"No models loaded. Use serve endpoint to load a model first.",
						);
					const {
						messages,
						model,
			...
			window.opener.postMessage(
				{
					...
					type: "ready",
					payload: { status: "ready", mode: "wllama", endpoints },
				},
				"*",
			);
	} catch {}
			...
			switch (type) {
				case "init": {
					...
					try {
						...
						console.log(
							"WebLLM engine created successfully (models loaded on-demand via reload)",
						);
						...
						if (
							...
							console.error(
								"WebLLM worker initialization failed - this is a fundamental limitation in iframe context",
							);
							console.error(
								"WebLLM requires WebAssembly workers which are restricted in iframe contexts",
							);
							console.error(
								"Recommendation: Use Wllama provider instead, which supports iframe contexts",
							);

							reply(src, origin, messageId, "error", {
								error: {
									message:
										"WebLLM is not compatible with iframe contexts due to WebAssembly worker restrictions. Please use the Wllama provider instead.",
									type: "WebWorkerError",
									code: "IFRAME_WEBLLM_INCOMPATIBLE",
									suggestion:
										"Switch to Wllama provider which supports iframe contexts",
								},
							});
						} else {
							reply(src, origin, messageId, "error", {
				...
				case "serve": {
					...
					const modelConfig = supportedModels.find(
						(m) => m.model_id === model || m.model === model,
					);

					if (!modelConfig) {
						const availableModels = supportedModels
							.map((m) => m.model_id || m.model)
							.join(", ");
						throw new Error(
							`Model '${model}' not found in supported models. Available: ${availableModels}`,
						);
					}
			...
			window.opener.postMessage(
				{
					messageId: "RUNNER_READY",
					type: "ready",
					payload: { status: "ready", mode: "webllm", endpoints },
				},
				"*",
			);
	} catch {}
