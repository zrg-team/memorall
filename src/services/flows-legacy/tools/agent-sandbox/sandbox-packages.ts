...
import type { SandboxPackageRequest } from "@/services/flows-legacy/interfaces/services/agent-sandbox";
...
const schema = z
	...
	.superRefine((data, ctx) => {
		const rules = {
			install: {
				allowed: ["packageSpec", "save", "saveDev"],
...
export const createSandboxPackagesTool: ToolFactory<Input, Services> = (
	services,
): Tool<Input> => ({
	name: TOOL_NAME,
	title: "Manage sandbox packages",
	description:
		"Install npm packages, install a workspace package.json, or list installed packages.",
	schema,
	...
	execute: (input, context) =>
		executeSandboxOperation(
			...
			context,
			(service, callContext) =>
				service.packages(input as SandboxPackageRequest, callContext),
		),
});
