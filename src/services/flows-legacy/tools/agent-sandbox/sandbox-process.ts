...
import type { SandboxProcessRequest } from "@/services/flows-legacy/interfaces/services/agent-sandbox";
...
const schema = z
	...
	.superRefine((data, ctx) => {
		const rules = {
			...
			read: {
				allowed: ["processId", "cursor", "waitMs", "maxChars"],
				required: ["processId"],
			},
			stdin: {
				allowed: ["processId", "input", "appendNewline"],
...
export const createSandboxProcessTool: ToolFactory<Input, Services> = (
	services,
): Tool<Input> => ({
	name: TOOL_NAME,
	title: "Manage sandbox processes",
	description:
		"List, read with an opaque cursor, write stdin to, or stop sandbox processes.",
	schema,
	...
	execute: (input, context) =>
		executeSandboxOperation(
			...
			context,
			(service, callContext) =>
				service.process(input as SandboxProcessRequest, callContext),
		),
});
