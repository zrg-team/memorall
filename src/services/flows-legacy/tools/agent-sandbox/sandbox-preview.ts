...
import type { SandboxPreviewRequest } from "@/services/flows-legacy/interfaces/services/agent-sandbox";
...
const schema = z.object({
	operation: z.enum(["start", "restart", "stop", "list", "request", "render"]),
	projectDir: z.string().min(1).optional(),
	kind: z.enum(["express", "vite", "next", "auto"]).optional(),
	template: z.enum(["express", "vite-react", "next-pages", "next-app"]).optional(),
	...
	const rules = {
		start: { allowed: ["projectDir", "kind", "template", "port", "entryPath", "hostname"], required: ["projectDir"] },
		restart: { allowed: ["projectDir", "kind", "template", "port", "entryPath", "hostname"], required: ["projectDir"] },
		stop: { allowed: ["previewId", "port"], required: [] },
		list: { allowed: [], required: [] },
		request: { allowed: ["previewId", "port", "path", "method", "headers", "body", "timeoutMs", "responseType", "maxChars"], required: [] },
		render: { allowed: ["previewId", "port", "path", "method", "headers", "body", "timeoutMs", "responseType", "maxChars"], required: [] },
	} as const;
	...
	if (["stop", "request", "render"].includes(data.operation) && data.previewId === undefined && data.port === undefined) {
		ctx.addIssue({ code: "custom", path: ["previewId"], message: "previewId or port is required" });
	}
});
type Input = z.infer<typeof schema>;
type Services = Pick<AllServices, "sandboxRuntime">;

export const createSandboxPreviewTool: ToolFactory<Input, Services> = (services): Tool<Input> => ({
	name: TOOL_NAME,
	title: "Manage sandbox previews",
	description: "Start, restart, stop, list, request, or render browser sandbox web previews.",
	schema,
	...
	execute: (input, context) => executeSandboxOperation(
		...
		input.operation,
		context,
		(service, callContext) => service.preview(input as SandboxPreviewRequest, callContext),
		input.operation === "render"
			? (result) => ({ ok: true, operation: input.operation, actionType: "web_access", result })
			: undefined,
	),
});
