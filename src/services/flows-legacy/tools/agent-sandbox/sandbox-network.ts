...
import type { SandboxNetworkRequest } from "@/services/flows-legacy/interfaces/services/agent-sandbox";
...
const schema = z.object({
	...
	validateOperationFields(data, ctx, ["url", "method", "headers", "body", "timeoutMs", "responseType", "maxChars"], ["url"]);
});
type Input = z.infer<typeof schema>;
type Services = Pick<AllServices, "sandboxRuntime">;

export const createSandboxNetworkTool: ToolFactory<Input, Services> = (services): Tool<Input> => ({
	name: TOOL_NAME,
	title: "Fetch from sandbox",
	description: "Fetch an HTTP or HTTPS resource from inside the active sandbox runtime.",
	schema,
	...
	execute: (input, context) => {
		...
		return executeSandboxOperation(
			...
			input.operation,
			context,
			(service, callContext) => service.network(request as SandboxNetworkRequest, callContext),
		);
	},
});
