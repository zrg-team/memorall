import type { QueryOptions, QueryResult, RowMode, RpcOp, RpcRequest, RpcResponse, RpcTransport, WorkerExecPayload, WorkerQueryPayload, WorkerQueryResult } from "./types";
...
/** PGlite-like surface expected by drizzle-orm/pglite */
export interface PGliteLike {
  query<R = unknown>(sql: string, params?: unknown[], options?: QueryOptions): Promise<QueryResult<R>>;
    ...
    const promise = new Promise<T>((resolve, reject) => {
      ...
      if (this.timeoutMs > 0) {
        ...
        waiter.timer = setTimeout(() => {
          this.pending.delete(id);
          reject(new Error(`RPC timeout after ${this.timeoutMs}ms for op "${op}"`));
        }, this.timeoutMs);
      }
 ...
 */
export function broadcastChannelTransport(channel: BroadcastChannel): RpcTransport {
  const handlers = new Set<(m: RpcResponse) => void>();
  const onMsg = (e: MessageEvent<RpcResponse>) => {
    if (!e.data || typeof e.data !== 'object') return;
    handlers.forEach((h) => h(e.data));
