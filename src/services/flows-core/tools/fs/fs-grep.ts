...
export const createFsGrepTool: ToolFactory<Input, Services> = (
	...
	name: TOOL_NAME,
	description:
		'Search file content with a JavaScript regex pattern. Returns results in grep-style "file:line:content" format. Supports context lines, minimatch glob filtering, and output modes. Use one broad regex and one broad glob when possible, e.g. pattern="icon|logo|brand" glob="**/*.{ts,tsx,md,json,svg,png}".',
	schema,
	execute: async (input) => {
		const {
			pattern,
