...
/**
 * How long a single `co-agent:*` command may take.
 *
 * This was two different numbers: the tools waited 10s while the extension
 * background gave up at 8s, so the background could abandon a command the tool
 * still wanted and the caller saw "Invalid co-agent browser response" instead of
 * a timeout. One constant now, with a ceiling for hosts that are legitimately
 * slower — a CDP round trip into the managed browser costs more than
 * a direct message to a content script in the same browser process.
 */
export const CO_AGENT_COMMAND_TIMEOUT_MS = 10_000;
export const CO_AGENT_COMMAND_TIMEOUT_MAX_MS = 30_000;
