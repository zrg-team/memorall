...
export const TEXT_TASK_META: Record<
	TextToolTask,
...
> = {
	"text-classification": {
		...
		can: "classify text",
		description:
			"Tell what a text is about or how it feels, with confidence scores.",
	},
	"zero-shot-classification": {
		icon: Tags,
		label: "Your labels",
		can: "sort text into your own labels",
		description: "Sort a text into labels you choose, no training needed.",
	},
	"text-ranking": {
		icon: ListOrdered,
		label: "Rank documents",
		can: "rank documents",
		description: "Order documents by how well they answer a query.",
	},
};

/** Translated task strings, with the English copy as the fallback. */
export function useTextTaskText() {
	const { t } = useTranslation("studioText");
	return (task: TextToolTask, field: "label" | "can" | "description") =>
		t(`tasks.${task}.${field}`, { defaultValue: TEXT_TASK_META[task][field] });
}

interface TextToolTaskSelectorProps {
	active: TextToolTask;
	supported: (task: TextToolTask) => boolean;
	onSelect: (task: TextToolTask) => void;
	isNarrow: boolean;
}

/**
 * The task picker in the composer toolbar. Every task stays listed even when
 * the current model cannot run it, so the list doubles as discovery; choosing
 * one explains how to get it.
 */
export const TextToolTaskSelector: React.FC<TextToolTaskSelectorProps> = ({
	active,
	supported,
	onSelect,
	isNarrow,
	...
	const ActiveIcon = TEXT_TASK_META[active].icon;
		...
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button
					...
					data-text-tool-task-trigger={active}
			...
			>
				<fieldset className="m-0 min-w-0 border-0 p-0" data-text-tool-tasks>
					<legend className="px-2 py-1.5 text-xs font-medium text-muted-foreground">
						{legend}
					</legend>
					{TEXT_TOOL_TASKS.map((task) => {
						const Icon = TEXT_TASK_META[task].icon;
...
export const TextToolSwitchModelHint: React.FC<{
	task: TextToolTask;
	className?: string;
}> = ({ task, className }) => {
	const { t } = useTranslation("studioText");
	...
	return (
		...
		>
			...
			<Button
				...
				onClick={() => navigate("/llm?category=text-tools")}
				data-switch-model
			>
