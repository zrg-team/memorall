...
`)}
    ...
    END;
    $$ LANGUAGE plpgsql;
  `;await e.exec(x)},V=async e=>{let t=z(ce),n=`
...
`)}
  ...
  `)},ve=async e=>{let t=z(f),n=z(se),r=z(x),i=z(p),a=z(_),o=`
    -- Create flows table
    ${t.table}
    ${t.indexes.join(`
`)}
...
`)}
    ...
    ${a.indexes.join(`
`)}
  `;await e.exec(o)},ye=async e=>{await e.exec(`
  ...
  `)},be=async e=>{await e.exec(`
	...
	`);let t=z(b);await e.exec(`
		...
		${t.indexes.join(`
`)}
	`);for(let t of C)await e.exec(t)},U=async e=>{await e.exec(`
  ...
  `,[t.id,t.version,t.description])}async function Ie(e){await Ne(e);let n=await Pe(e);for(let i of G)if(n.includes(i.id))r(`⏭️ Migration ${i.id} already applied`);else{r(`Running migration: ${i.id} - ${i.description}`);try{await i.up(e),await Fe(e,i),r(`✅ Migration ${i.id} completed successfully`)}catch(e){throw t(`❌ Migration ${i.id} failed:`,e),e}}}var K=null,q=null,J=null;async function Le(e){if(q)return q;try{let t=typeof e==`string`?{mode:E.MAIN,dataDir:e}:e||{mode:E.MAIN};if(J=t.mode,t.mode===E.MAIN){let e=new O(t.dataDir||`idb://memorall-db`,{extensions:{vector:M,uuid_ossp:j,pg_trgm:N}});await e.waitReady,K=e,await Ie(e),n(`✅ Database initialized in MAIN mode`)}else throw Error(`PROXY mode should not initialize database here. Use DatabaseServiceProxy instead.`);return q=D(K,{schema:w}),n(`✅ Database initialized successfully in ${t.mode.toUpperCase()} mode`),q}catch(e){throw t(`❌ Database initialization failed:`,e),e}}function Re(){if(!q)throw Error(`Database not initialized. Call initDB() first.`);return q}function ze(){if(!K)throw Error(`Database not initialized. Call initDB() first.`);return K}function Y(){return J}function X(){return J===E.MAIN}function Z(){return J===E.PROXY}async function Q(){try{return K?{healthy:!0,test:(await K.query(`SELECT 1 as test`)).rows[0],mode:J}:{healthy:!1,error:`Database not initialized`}}catch(e){return{healthy:!1,error:e instanceof Error?e.message:`Unknown error`,mode:J}}}async function Be(){K&&(await K.close(),K=null,q=null,J=null)}var Ve={startListening:()=>void 0,stop:()=>void 0};function $(){return Ve}var He=class extends k{async initializeDatabase(){n(`📚 Initializing database service in "MAIN" mode. Channel: "${this.config.proxyOptions?.channelName}"`);try{await Le(this.config);let e=this.config.proxyOptions?.channelName;e&&$().startListening(e),n(`✅ Database service initialized successfully`)}catch(e){throw t(`❌ Database service initialization failed:`,e),e}}hasTable(e){return e in w}getTableNames(){return Object.keys(w)}async getStatus(){let e={initialized:this.initialized,mode:Y(),isMainMode:X(),isProxyMode:Z(),tableCount:Object.keys(w).length,availableTables:this.getTableNames(),healthy:!1,healthCheck:null};if(this.initialized)try{e.healthCheck=await Q(),e.healthy=e.healthCheck.healthy}catch(t){e.healthCheck={healthy:!1,error:t instanceof Error?t.message:`Unknown error`}}return e}getMode(){return Y()}isMainMode(){return X()}isProxyMode(){return Z()}async healthCheck(){try{return await this.ensureInitialized(),(await Q()).healthy}catch(e){return t(`❌ Database health check failed:`,e),!1}}async close(){this.config?.mode===E.MAIN&&this.config.proxyOptions?.channelName&&($().stop(),n(`📡 RPC handler stopped`)),await Be(),this.initialized=!1,this.initPromise=null,this.config=null,n(`📚 Database service closed`)}async use(e,t){await this.ensureInitialized();let n=Re(),r=ze(),i={db:n,schema:w,raw:(e,t)=>r.query(e,t)};return t?.transaction?n.transaction(async t=>e({db:t,schema:w,raw:(e,t)=>r.query(e,t)})):e(i)}};export{He as DatabaseServiceMain};